...
import {
  createToolResponse,
  formatBookmarksResult,
} from "../../utils/responses.js"; // Assuming formatBookmarksResult will be added
...
async function readBookmarks(
  ...
  try {
    ...
    if (!bookmarksData || !Array.isArray(bookmarksData.items)) {
      throw new Error(
        "Invalid bookmarks.json structure: 'items' array not found or invalid.",
      );
    }
    ...
    if (error instanceof SyntaxError) {
      throw new McpError(
        ...
        `Error parsing bookmarks.json: ${error.message}`,
      );
    }
...
- List bookmarks for vault: { "vault": "my_vault" }`,
      ...
      handler: async (args, vaultPath, _vaultName) => {
        try {
          ...
          console.error(
            `Error listing bookmarks for vault '${args.vault}':`,
            error,
          );
