...
const schema = z.object({
  ...
  path: z.string()
    .optional()
    .refine(dirPath => !dirPath || !path.isAbsolute(dirPath),
      "Directory path must be relative to vault root")
    .describe("Optional sub-directory path within the vault to list files from (relative to vault root)")
}).strict();
...
export function createListFilesTool(vaults: Map<string, string>) {
  return createTool<ListFilesInput>({
    name: "list-files",
    description: "Lists non-Markdown files (like images, PDFs) in the vault or a specific folder.",
    schema,
