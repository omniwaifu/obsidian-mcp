...
const schema = z.object({
  ...
  path: z.string()
    ...
    .describe("Relative path of the note containing the task (e.g., 'folder/note.md')"),
  line: z.number()
    .int()
    .positive("Line number must be a positive integer")
    .describe("The 1-based line number of the task to toggle (e.g., obtained from get-tasks-in-note)"),
}).strict();
...
async function toggleTaskStatus(
  ...
  if (!(await fileExists(noteAbsPath))) {
    throw new McpError(ErrorCode.InvalidParams, `Note not found: ${noteRelPath}`);
  }
  ...
  try {
    ...
    if (targetLineNumber <= 0 || targetLineNumber > lines.length) {
        throw new McpError(ErrorCode.InvalidParams, `Invalid line number: ${targetLineNumber}. Note has ${lines.length} lines.`);
    }
    ...
    if (!match) {
        throw new McpError(ErrorCode.InvalidParams, `Line ${targetLineNumber} is not a valid Markdown task: "${lineContent.substring(0, 50)}..."`);
    }
...
- Toggle task on line 5: { "vault": "my_vault", "path": "todos.md", "line": 5 }`,
    ...
    handler: async (args, vaultPath, _vaultName) => {
      try {
        ...
        console.error(`Error toggling task in '${args.path}' (line ${args.line}) in vault '${args.vault}':`, error);
