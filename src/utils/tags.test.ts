...
describe('Tag Utilities', () => {
  ...
  describe('normalizeTag', () => {
    ...
    it('should not normalize if normalize flag is false', () => {
      ...
      expect(normalizeTag('#simpleTag', false)).toBe('simpleTag');
      expect(normalizeTag('With/HierarchyTag', false)).toBe('With/HierarchyTag');
    });
  });
  ...
  describe('parseNote', () => {
    it('should parse note with frontmatter', () => {
      ...
      expect(parsed.frontmatter).toEqual({ key: 'value', tags: ['tag1', 'tag2'], aliases: ['alias1'] });
...
---`; // Use backticks
  ...
  describe('stringifyNote', () => {
    it('should stringify note with frontmatter', () => {
      ...
      expect(stringifyNote(parsed).replace(/\s+/g, ' ')).toBe(expected.replace(/\s+/g, ' '));
    });
...
End #tag3
      `;
      expect(extractTags(content).sort()).toEqual(['tag1', 'tag2', 'tag3'].sort());
    });
  });
  ...
  describe('matchesTagPattern', () => {
      ...
      it('should match patterns with wildcards', () => {
       ...
       it('should match hierarchical patterns with wildcards', () => {
          ...
          expect(matchesTagPattern('project/*/tasks', 'project/alpha/tasks')).toBe(true);
  ...
  describe('getRelatedTags', () => {
    ...
    it('should get children tags', () => {
        expect(getRelatedTags('a', allTags).children.sort()).toEqual(['a/b', 'a/d'].sort());
        expect(getRelatedTags('a/b', allTags).children.sort()).toEqual(['a/b/c'].sort());
        expect(getRelatedTags('b', allTags).children.sort()).toEqual(['b/c'].sort());
        expect(getRelatedTags('c', allTags).children.sort()).toEqual([].sort()); // No children
    });
     it('should get parent tags', () => {
        expect(getRelatedTags('a/b/c', allTags).parents.sort()).toEqual(['a', 'a/b'].sort());
        expect(getRelatedTags('a/d', allTags).parents.sort()).toEqual(['a'].sort());
        expect(getRelatedTags('b/c', allTags).parents.sort()).toEqual(['b'].sort());
