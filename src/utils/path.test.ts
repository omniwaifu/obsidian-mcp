...
const mockExecFn = mock(async (_command: string, _options?: any): Promise<{ stdout: string; stderr: string }> => {
...
const resetMockImplementations = () => {
  ...
  mockFsStat.mockImplementation(async (_p: string) => ({ isDirectory: () => true }));
  mockExecFn.mockImplementation(async (_command: string, _options?: any): Promise<{ stdout: string; stderr: string }> => {
...
describe('checkPathCharacters', () => {
  ...
  it('should detect path too long', () => {
    ...
    runWithPlatform('win32', () => {
      ...
      expect(checkPathCharacters(longWinPath)).toMatch(/exceeds maximum length/);
    });
  });
  ...
  it('should detect component too long', () => {
    runWithPlatform('linux', () => {
      ...
      expect(checkPathCharacters(longComponent)).toMatch(/Directory\/file name too long/);
    });
    runWithPlatform('win32', () => {
      const longWinComponent = 'C:\\' + 'a'.repeat(256);
      expect(checkPathCharacters(longWinComponent)).toMatch(/Directory\/file name too long/);
    });
  });

  it('should detect drive root paths (Windows)', () => {
    runWithPlatform('win32', () => {
      expect(checkPathCharacters('C:\\')).toBe('Cannot use drive root directory');
      expect(checkPathCharacters('d:')).toBe('Cannot use drive root directory');
      expect(checkPathCharacters('Z:\\')).toBe('Cannot use drive root directory');
  ...
  it('should detect filesystem root path (Unix)', () => {
    runWithPlatform('linux', () => {
      expect(checkPathCharacters('/')).toBe('Cannot use filesystem root directory');
  ...
  it('should detect relative path components', () => {
    ...
    expect(checkPathCharacters('/path/./other')).toMatch(/relative components/);
    expect(checkPathCharacters('C:\\path\\..\\other')).toMatch(/relative components/);
  });

  it('should detect non-printable characters', () => {
    expect(checkPathCharacters('/path/with\x01char')).toMatch(/non-printable characters/);
    expect(checkPathCharacters('C:\\path\\with\x7f')).toMatch(/non-printable characters/);
  });

  it('should detect Windows reserved names', () => {
    runWithPlatform('win32', () => {
      expect(checkPathCharacters('C:\\CON')).toMatch(/Windows reserved names/);
      expect(checkPathCharacters('C:\\folder\\PRN.txt')).toMatch(/Windows reserved names/);
      expect(checkPathCharacters('C:\\folder\\COM1')).toMatch(/Windows reserved names/);
      expect(checkPathCharacters('C:\\folder\\LPT9.doc')).toMatch(/Windows reserved names/);
  ...
  it('should detect invalid Windows characters', () => {
    runWithPlatform('win32', () => {
      expect(checkPathCharacters('C:\\folder<invalid>')).toMatch(/not allowed on Windows/);
      expect(checkPathCharacters('C:\\folder:invalid')).toMatch(/not allowed on Windows/);
      expect(checkPathCharacters('C:\\folder\"invalid\"')).toMatch(/not allowed on Windows/);
      expect(checkPathCharacters('C:\\folder|invalid')).toMatch(/not allowed on Windows/);
      expect(checkPathCharacters('C:\\folder?invalid')).toMatch(/not allowed on Windows/);
      expect(checkPathCharacters('C:\\folder*invalid')).toMatch(/not allowed on Windows/);
      expect(checkPathCharacters('C:\\valid:folder')).toMatch(/not allowed on Windows/);
      expect(checkPathCharacters('C:\\valid\\folder')).toBeNull();
      expect(checkPathCharacters('\\\\server\\share\\folder:invalid')).toMatch(/not allowed on Windows/);
      runWithPlatform('linux', () => {
        expect(checkPathCharacters('/path/to/file:with:colons')).toBeNull();
        expect(checkPathCharacters('/path/to/file<with>other?chars*')).toBeNull();
      });
    });
  });
  ...
  it('should detect Windows device paths', () => {
    runWithPlatform('win32', () => {
      expect(checkPathCharacters('\\\\.\\C:')).toMatch(/Device paths are not allowed/);
      expect(checkPathCharacters('\\\\.\\PhysicalDrive0')).toMatch(/Device paths are not allowed/);
    });
  });

  it('should detect invalid Unix characters (null byte)', () => {
    runWithPlatform('linux', () => {
      expect(checkPathCharacters('/path/with\x00null')).toMatch(/Contains null characters/);
    });
    runWithPlatform('win32', () => {
  ...
  it('should detect invalid Unicode characters (replacement char)', () => {
    expect(checkPathCharacters('/path/with\uFFFDreplacement')).toMatch(/invalid Unicode characters/);
  });

  it('should detect leading/trailing whitespace', () => {
    expect(checkPathCharacters(' /path/with/leading/space')).toMatch(/leading or trailing whitespace/);
    expect(checkPathCharacters('/path/with/trailing/space ')).toMatch(/leading or trailing whitespace/);
    expect(checkPathCharacters('/path/ with /space ')).toMatch(/leading or trailing whitespace/);
    expect(checkPathCharacters('C:\\trailing\\space ')).toMatch(/leading or trailing whitespace/);
  });

  it('should detect consecutive separators', () => {
    expect(checkPathCharacters('/path//with/double/slash')).toMatch(/consecutive path separators/);
    expect(checkPathCharacters('C:\\\\folder')).toMatch(/consecutive path separators/);
    expect(checkPathCharacters('C:\\folder\\\\another')).toMatch(/consecutive path separators/);
  });
});
...
describe('checkLocalPath', () => {
  ...
  const runWithPlatform = (platform: NodeJS.Platform, testFn: () => void | Promise<void>) => {
    Object.defineProperty(process, 'platform', { value: platform, writable: true });
    return Promise.resolve().then(testFn).finally(() => {
        Object.defineProperty(process, 'platform', { value: originalPlatform, writable: true });
    });
  };
  ...
  it('should detect symlink pointing outside parent', async () => {
    ...
    expect(await checkLocalPath('/home/user/vault')).toMatch(/symlinks that point outside/);
  });
  ...
  it('should handle realpath ELOOP error (circular symlink)', async () => {
    ...
    expect(await checkLocalPath('/home/user/vault')).toMatch(/circular symlinks/);
  });

  it('should detect Windows network drive (UNC - TEST ENV)', async () => {
    await runWithPlatform('win32', async () => {
        mockFsRealpath.mockResolvedValue('\\\\Server\\Share\\folder');
        expect(await checkLocalPath('Z:\\folder')).toMatch(/Network, removable, or unknown drive type/);
    });
  });
  ...
  it('should detect Unix network mount (NFS - TEST ENV)', async () => {
    await runWithPlatform('linux', async () => {
        ...
        expect(await checkLocalPath('/home/user/linked_nfs_share')).toMatch(/Network or remote filesystem/);
    });
  });
  ...
  it('should detect Unix network mount (CIFS - TEST ENV)', async () => {
    await runWithPlatform('linux', async () => {
        ...
        expect(await checkLocalPath('/home/user/linked_smb_share')).toMatch(/Network or remote filesystem/);
    });
  });
  ...
  it('should handle df error gracefully (Unix - TEST ENV)', async () => {
    await runWithPlatform('linux', async () => {
        ...
        expect(await checkLocalPath('/home/user/linked_unknown')).toMatch(/Network or remote filesystem/);
    });
  });
   ...
   it('should handle df timeout (Unix - TEST ENV)', async () => {
      await runWithPlatform('linux', async () => {
          ...
          expect(await checkLocalPath('/home/user/linked_timeout')).toMatch(/Network or remote filesystem/);
      });
  });
});
...
describe('checkSuspiciousPath', () => {
  ...
  it('should return null for valid vault paths', async () => {
    expect(await checkSuspiciousPath('/home/user/Documents/MyVault')).toBeNull();
    expect(await checkSuspiciousPath('/home/user/Documents/MyVault/.obsidian')).toBeNull();
    expect(await checkSuspiciousPath('C:\\Users\\Test\\Documents\\MyVault')).toBeNull();
    expect(await checkSuspiciousPath('C:\\Users\\Test\\MyVault\\.obsidian')).toBeNull();
  });

  it('should detect hidden directories (except .obsidian)', async () => {
    await runWithPlatform('linux', async () => {
        expect(await checkSuspiciousPath('/home/user/.hidden/vault')).not.toBeNull();
        expect(await checkSuspiciousPath('/home/user/vault/.config')).not.toBeNull();
    });
    await runWithPlatform('win32', async () => {
        expect(await checkSuspiciousPath('C:\\Users\\Test\\.hidden\\')).not.toBeNull();
    });
  });
  ...
  it('should detect system directories', async () => {
    ...
    expect(await checkSuspiciousPath('C:\\Program Files\\SomeApp')).not.toBeNull();
...
describe('checkPathOverlap', () => {
  it('should throw McpError for duplicate paths', () => {
    ...
    expect(() => checkPathOverlap(['C:/a/b', 'C:/c/d', 'C:/a/b'])).toThrow(McpError);
  });

  it('should throw McpError for overlapping paths (parent/child)', () => {
    expect(() => checkPathOverlap(['/a/b', '/c/d', '/a/b/c'])).toThrow(McpError);
    expect(() => checkPathOverlap(['/a/b/c', '/c/d', '/a/b'])).toThrow(McpError);
  });
  ...
  it('should normalize paths before checking', () => {
    expect(() => checkPathOverlap(['/a/b/', '/c/d', '/a/b'])).toThrow(McpError);
    expect(() => checkPathOverlap(['/a/b/c/..', '/c/d', '/a/b/e'])).toThrow(McpError);
    expect(() => checkPathOverlap(['/a/b/c/../', '/c/d', '/a/b/e'])).toThrow(McpError);
    expect(() => checkPathOverlap(['/a/b/c/..', '/c/d', '/a/b'])).toThrow(McpError);
  });
});
...
describe('checkPathSafety', () => {
    ...
    it('should return error from checkPathCharacters', async () => {
        ...
        if (result === null) {
            throw new Error('Expected an error string for checkPathCharacters, got null');
        }
    ...
    it('should return error from checkLocalPath (Windows - TEST ENV UNC)', async () => {
        await runWithPlatform('win32', async () => {
            ...
            if (result === null) {
                throw new Error('Expected an error string for checkLocalPath, got null');
            }
    ...
    it('should return error from checkSuspiciousPath', async () => {
        ...
        if (result === null) {
            throw new Error('Expected an error string for checkSuspiciousPath, got null');
        }
    ...
    it('should return error if path does not exist', async () => {
        ...
        if (result === null) {
            throw new Error('Expected an error string for non-existent path, got null');
        }
    ...
    it('should return error if path is not a directory', async () => {
        ...
        if (result === null) {
            throw new Error('Expected an error string for non-directory path, got null');
        }
