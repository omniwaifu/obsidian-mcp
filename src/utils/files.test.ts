...
describe("File Utilities", () => {
  ...
  beforeAll(async () => {
    ...
    await fs.writeFile(
      path.join(vaultPath, "SubFolder", "subnote1.md"),
      "subcontent1",
    );
    await fs.writeFile(path.join(vaultPath, "SubFolder", "data.json"), "{}");
    await fs.writeFile(
      path.join(vaultPath, ".hiddenFolder", "hidden_note.md"),
      "hidden_md",
    );
    await fs.writeFile(
      path.join(vaultPath, ".hiddenFolder", "hidden_data.txt"),
      "hidden_txt",
    );
    await fs.writeFile(
      path.join(vaultPath, ".obsidian", "config"),
      "obsidian_config",
    );
  ...
  describe("getAllMarkdownFiles", () => {
    ...
    it("should throw error for search directory outside vault", async () => {
      ...
      await expect(
        getAllMarkdownFiles(vaultPath, outsidePath),
      ).rejects.toThrow();
    });
  });
  ...
  describe("getAllNonMarkdownFiles", () => {
    it("should find all non-markdown files recursively, excluding hidden/system", async () => {
      ...
      expect(relativeFiles).toEqual(
        ["SubFolder/data.json", "document.pdf", "image.png"].sort(),
      );
    });
  ...
  describe("fileExists", () => {
    ...
    it("should return false for a non-existent file", async () => {
      expect(await fileExists(path.join(vaultPath, "nonexistent.txt"))).toBe(
        false,
      );
    });
  ...
  describe("ensureDirectory", () => {
    ...
    afterEach(async () => {
      ...
      await fs
        .rm(path.join(vaultPath, "Nested"), { recursive: true, force: true })
        .catch(() => {});
    });
  ...
  describe("safeReadFile", () => {
    ...
    it("should return undefined for a non-existent file", async () => {
      const content = await safeReadFile(
        path.join(vaultPath, "nonexistent.txt"),
      );
    ...
    it("should throw McpError when trying to read a directory", async () => {
      ...
      await expect(
        safeReadFile(path.join(vaultPath, "SubFolder")),
      ).rejects.toThrow();
