...
function isHiddenOrSystem(fullPath: string, vaultPath: string): boolean {
  ...
  return parts.some(part => part.length > 0 && part.startsWith('.') && part !== '.obsidian');
}
  ...
  try {
    ...
    if ((error as any).code === 'ENOENT') {
      ...
      if (normalizedDir !== normalizedVaultPath) {
        console.warn(`Directory not found during markdown file scan (ignoring): ${normalizedDir}`);
        return [];
      }
      // Re-throw if vaultPath itself doesn't exist
      throw new McpError(ErrorCode.InvalidRequest, `Vault directory not found: ${normalizedVaultPath}`);
    }
    // Rethrow other readdir errors
    throw new McpError(ErrorCode.InternalError, `Failed to read directory ${normalizedDir}: ${error instanceof Error ? error.message : String(error)}`);
  }
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      if (error instanceof McpError) {
        console.error(`Skipping ${entry.name} in ${normalizedDir}:`, error.message);
      } else {
        console.error(`Error processing ${entry.name} in ${normalizedDir}:`, error);
      }
...
export async function getAllNonMarkdownFiles(
  ...
  if (!targetDir.startsWith(normalizedVaultPath)) {
      throw new McpError(ErrorCode.InvalidRequest, 'Search directory must be inside the vault path');
  }
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
      ...
      if (targetDir !== normalizedVaultPath) {
        console.warn(`Directory not found during non-markdown file scan (ignoring): ${targetDir}`);
        return [];
      }
      throw new McpError(ErrorCode.InvalidRequest, `Vault directory not found: ${normalizedVaultPath}`);
    }
    console.error(`Error reading directory ${targetDir}:`, error);
    throw new McpError(ErrorCode.InternalError, `Failed to read directory: ${targetDir}`);
  }
...
export async function safeReadFile(filePath: string): Promise<string | undefined> {
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === 'EISDIR') {
       ...
       throw new McpError(ErrorCode.InvalidRequest, `Path is a directory, not a file: ${filePath}`);
    }
    console.error(`Error reading file ${filePath}:`, error);
    throw new McpError(ErrorCode.InternalError, `Failed to read file: ${filePath}`);
  }
}
