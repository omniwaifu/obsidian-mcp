...
describe("E2E: get-daily-note-path Tool (Internal Server Test)", () => {
  ...
  beforeAll(async () => {
    ...
    callToolHandler = async (request: any) => {
      ...
      try {
        ...
        throw new McpError(ErrorCode.InternalError, `Unexpected test error: ${error}`);
      }
    };
  });
  ...
  it("should return the correct daily note path based on config", async () => {
    ...
    try {
      ...
      expect(response.content[0].text).toBe(`Daily note path: ${expectedRelativePath}`); 
    } catch (error) {
  ...
  it("should return error if vault name is invalid", async () => {
      ...
      try {
          ...
          expect(true).toBe(false); // Should not reach here
           ...
           expect((error as McpError).message).toContain("Unknown vault: non_existent_vault"); 
      }
  });
